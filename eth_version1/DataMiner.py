...
class DataMiner():
    ...
    def __init__(self, date, crypto, symbol):
        self.date = date
    ...
    def __call__(self):
        # date, search popularity, number of news articles, crypto price
        self.row = [self.date, self.search_pop, self.news_articles, self.news_sentiment, self.reddit_posts, self.reddit_sentiment, self.price]


    # News API data
    def crypto_news_query(self):
        if self.date is not None:
            # get single day results
            fromdate = 'from=' + self.date
            todate = 'to=' + self.date
            headers = {'X-Api-Key': news_api_key}
            url = 'https://newsapi.org/v2/everything?q=' + self.crypto + '&from=' + fromdate + "&to=" + todate
            response = requests.get(url, headers=headers)
            response = response.json()
            return response['totalResults']
        else:
            raise TypeError('Must supply date to crypto_news_query')
    ...
    def get_crypto_price(self):
        ...
        url = api_url + '/products/' + self.symbol + '-USD/candles?start=' + self.date + '&end=' + enddate + '&granularity=3600'
    ...
    def search_reddit(self):
        ...
        response = requests.get("https://oauth.reddit.com/search?q=" + self.crypto + "&t=day&sort=top", headers=headers)
